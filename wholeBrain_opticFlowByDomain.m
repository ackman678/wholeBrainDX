function [Vsum, AVx, AVy] = wholeBrain_opticFlowByDomain(A,region,fnm)
...
for fr = frStart:frEnd-1
	for k = 1:nDomains
		if fr >= onsets(k) & fr <= offsets(k) & onsets(k) >= frStart & offsets(k) <= frEnd
...
for fr = frStart:frEnd-1
	...
	img1 = A(:,:,fr);
	% imagesc(img1,clims); set(gca,'ydir','reverse'); colormap(gray)
	...
	for k = 1:nDomains
		if fr >= onsets(k) & fr <= offsets(k) & onsets(k) >= frStart & offsets(k) <= frEnd
			...
			[dxpts,dypts] = pol2cart(theta,(rho./mx).*arrowLength);
			% amquiver(X, Y, Vx, Vy);
