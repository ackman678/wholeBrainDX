...
%BSD license
%Downloaded from matlabcentral by James B. Ackman 2013-04-16
%Edited by JBA to work together with legendflex.m 2013-04-17 07:50:44
...
[legend_h,object_h,plot_h,text_str] = legendflex(varargin{1}, varargin{2},'ref', varargin{1}, 'anchor', [3 1], 'buffer', [ 10   0]);
leghan = legend_h;
objhan = object_h;
plothan = plot_h;

if length(varargin) > 2 || ~isempty(varargin{3})
    hControl = varargin{3};  %for the call to uicontrol in 'togglevisibility'  
else
    hControl = [];
end

% [varargout{1:nargout(@legendflex)}] = legendflex(varargin{1}, varargin{2},'ref', varargin{1}, 'anchor', [3 1], 'buffer', [ 10   0]);

% Extract what is needed for the rest of the function and fix varargout
%   [leghan, objhan, plothan] = varargout{1:3};   %JBA edit
% objhan: strings
...
for i = 1:length(plothan)
    set(objhan(i), 'HitTest', 'on', 'ButtonDownFcn',...
        @(varargin)togglevisibility(objhan(i),obj(groupmem==groups(i)),hControl),...
        'UserData', true);
    if dispflag && hidden(i)
        togglevisibility(objhan(i), obj(groupmem==groups(i)), hControl);
    end
end


function togglevisibility(hObject, obj, hControl)
if get(hObject, 'UserData') % It is on, turn it off
    ...
    if ~isempty(hControl), uicontrol(hControl); end
else
