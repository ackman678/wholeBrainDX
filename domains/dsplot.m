function hL = dsplot(x, y, numPoints, hFig, hAxes)  %JBA edit
...
%--------------------------------------------------------------------------
% Error checking
error(nargchk(1, 5, nargin, 'struct'));  %JBA edit
if nargin < 3 || isempty(numPoints)  %JBA edit
...
if nargin < 4 || isempty(hFig), hFig = figure; end  %JBA edit
if nargin < 5 || isempty(hAxes), hAxes = gca; end %JBA edit

figName = '';

% Create template plot using NaNs
hLine   = plot(hAxes, NaN(2, numSignals), NaN(2, numSignals));
set(hLine, 'tag', 'dsplot_lines');

% Define CreateFcn for the figure
set(hFig, 'CreateFcn', @mycreatefcn);
mycreatefcn(hFig,hAxes);

%{
%commented out by JBA 
...
%     lenVarargin = length(varargin);
    ...
    if nargin > 1
        ...
        hLine = findobj(hFig, 'type', 'axes'); %whos('hLine')
        axesHandle = findobj(hFig, 'type', 'axes'); %whos('hLine')
        hLine(strmatch('legend', get(hLine, 'tag'))) = [];
    end
    hLine = get(axesHandle, 'Children'); %whos('hLine')
...
%--------------------------------------------------------------------------
  function updateLines(rng)
    ...
    if length(id) > numPoints / numSignals
      ...
      idid = round(linspace(id(1), id(end), round(numPoints/numSignals)))';%do the actual downsampling
    ...
    % Update plot
    set(hLine, {'xdata', 'ydata'} , [x2, y2]);   %here the line object is updated with the down-sampled datapoints
%     set(hFig, 'Name', figName);  %JBA edit
